      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>
              {isBoardMember ? "Official published records for your meetings." : "Manage drafts, reviews, and published meeting records."}
            </p>
          </div>
        ...
        <div className="space-y-3">
          {minutesList.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center text-center rounded-2xl border" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)" }}>
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4" style={{ background: "linear-gradient(135deg,rgba(99,102,241,0.1),rgba(124,58,237,0.1))", border: "1px solid rgba(99,102,241,0.2)" }}>
              ...
              <p className="text-sm max-w-sm" style={{ color: "var(--text-muted)" }}>
                {isBoardMember ? "No official minutes have been published yet." : "Generate minutes from a completed meeting to see them here."}
              </p>
            </div>
          ) : (
            minutesList.map((m: any) => (
              <Link key={m._id.toString()} href={`/minutes/${m._id}`}>
                <div className="group p-5 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:bg-white/[0.02]" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)" }}>
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-1" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--border-subtle)" }}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-2 text-xs" style={{ color: "var(--text-muted)" }}>
                        <span>Meeting: {m.meetingId?.scheduledAt ? new Date(m.meetingId.scheduledAt).toLocaleDateString() : "No date"}</span>
                        <span className="w-1 h-1 rounded-full bg-white/20" />
                        <span>Drafted by {(m.draftedBy as any)?.name || "Unknown"}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-sm font-600 text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                    View Details &rarr;
                  </div>
                </div>
              </Link>
...
function StatCard({ title, value, icon: Icon, color, bg, border }: any) {
  return (
    <div className="p-5 rounded-2xl border" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)" }}>
...
function FilterTab({ label, active, href }: { label: string; active: boolean; href: string }) {
  return (
    <Link 
      href={href}
      className={`px-4 py-3 text-sm font-600 border-b-2 transition-colors ${active ? "border-indigo-500 text-indigo-400" : "border-transparent text-white/50 hover:text-white"}`}
    >
      {label}
    </Link>
  );
