...
import {
  ...
  Calendar, MapPin, User, ShieldCheck, Clock
} from "lucide-react";
  ...
  return (
    <AppShell title={`Minutes: ${meeting?.title || "Meeting"}`}>
      <div className="max-w-5xl mx-auto space-y-4 pb-16">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isDraftOrReview && !isEditing && canEdit && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-white dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-xs font-600 text-slate-800 dark:text-white transition-all cursor-pointer shadow-xs"
              >
            ...
            {isEditing && canEdit && (
              <button
                onClick={handleSave}
                disabled={saving}
                className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl btn-gradient keep-white text-xs font-600 transition-all disabled:opacity-60 cursor-pointer shadow-md shadow-purple-500/20"
              >
            ...
            {isDraftOrReview && !isEditing && canApprove && (
              <button
                ...
                disabled={approving}
                className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-600 transition-all disabled:opacity-60 cursor-pointer shadow-md shadow-emerald-500/20"
              >
            ...
            {isApproved && canApprove && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white text-xs font-600 transition-all cursor-pointer shadow-md shadow-amber-500/20"
              >
        ...
        {errorMsg && (
          <div className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-rose-50 dark:bg-red-500/10 border border-rose-200 dark:border-red-500/20 text-xs font-600 text-rose-700 dark:text-red-300">
            <AlertTriangle size={15} className="shrink-0 text-rose-600 dark:text-red-400" />
            {errorMsg}
          </div>
        )}
        ...
        {isBoardMember && !isPublished && (
          <div className="flex items-start gap-2.5 p-3 rounded-xl bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/20">
            <FileText size={16} className="text-purple-600 dark:text-purple-400 mt-0.5 shrink-0" />
            <div>
        ...
        {minutes.generatedByAI && !isPublished && !isBoardMember && (
          <div className="flex items-start gap-2.5 p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
        ...
        {/* ── Main Document Card (Compact Layout) ── */}
        <div className="rounded-2xl border border-slate-200/80 dark:border-white/[0.08] p-5 sm:p-6 space-y-6 bg-white dark:bg-white/[0.02] shadow-xs relative overflow-hidden">
          
          {/* Top Decorative Accent Line */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-indigo-500 to-fuchsia-500" />

          {/* Document Header */}
          <div className="border-b border-slate-200/80 dark:border-white/[0.08] pb-4 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
            ...
            <h1 className="text-xl sm:text-2xl font-700 text-slate-900 dark:text-white tracking-tight leading-tight">
              {meeting?.title || "Board Meeting"}
            </h1>

            {/* Metadata Grid (Compact Bar) */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-1">
              <MetaCard icon={<Calendar size={13} className="text-purple-600 dark:text-purple-400" />} label="Date" value={meetingDateStr} />
              <MetaCard icon={<Clock size={13} className="text-indigo-600 dark:text-indigo-400" />} label="Type" value={meeting?.meetingType || "Board Meeting"} />
              <MetaCard icon={<MapPin size={13} className="text-blue-600 dark:text-blue-400" />} label="Location" value={meeting?.location || meeting?.onlineMeeting || "Virtual"} />
              <MetaCard icon={<User size={13} className="text-emerald-600 dark:text-emerald-400" />} label="Drafted By" value={minutes.draftedBy?.name || "Unknown"} />
              <MetaCard icon={<ShieldCheck size={13} className="text-fuchsia-600 dark:text-fuchsia-400" />} label="Approved By" value={minutes.approvedBy?.name || "Pending"} />
              {minutes.approvedAt && (
                <MetaCard icon={<Clock size={13} className="text-slate-600 dark:text-white/50" />} label="Approved At" value={new Date(minutes.approvedAt).toLocaleDateString()} />
              )}
              {minutes.publishedAt && (
                <MetaCard icon={<Clock size={13} className="text-slate-600 dark:text-white/50" />} label="Published" value={new Date(minutes.publishedAt).toLocaleDateString()} />
              )}
            </div>
          </div>

          {/* Meeting Summary */}
          <Section title="Meeting Summary" icon={<FileText size={15} />}>
            {isEditing ? (
              <textarea
                className="w-full h-24 p-2.5 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-300 dark:border-white/10 text-xs text-slate-900 dark:text-white outline-none focus:border-purple-500 resize-none font-500"
          ...
          {/* Attendance */}
          <Section title="Attendance" icon={<CheckCircle2 size={15} />}>
            <div className="rounded-xl border border-slate-200/80 dark:border-white/[0.08] overflow-hidden shadow-2xs">
              <table className="w-full text-xs text-left">
                <thead className="bg-slate-100/90 dark:bg-white/[0.04] border-b border-slate-200/80 dark:border-white/[0.08]">
                  <tr>
                    <th className="px-3.5 py-2 font-700 text-slate-700 dark:text-white/80">Name</th>
                    <th className="px-3.5 py-2 font-700 text-slate-700 dark:text-white/80">Role</th>
                    <th className="px-3.5 py-2 font-700 text-slate-700 dark:text-white/80">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200/60 dark:divide-white/[0.06] bg-white dark:bg-transparent">
                  {editedData.attendees?.map((a: any, i: number) => (
                    <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                      <td className="px-3.5 py-2 font-600 text-slate-900 dark:text-white">{a.name}</td>
                      <td className="px-3.5 py-2 font-500 text-slate-600 dark:text-white/60 capitalize">{a.role ? a.role.replace(/_/g, " ") : "-"}</td>
                      <td className="px-3.5 py-2">
                        {isEditing ? (
                          <select
                            className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-white/20 rounded px-2 py-0.5 text-xs text-slate-900 dark:text-white"
                  ...
                  {(!editedData.attendees || editedData.attendees.length === 0) && (
                    <tr><td colSpan={3} className="px-3.5 py-4 text-center text-slate-500 dark:text-white/40">No attendance data recorded.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {/* Agenda & Discussions */}
          <Section title="Agenda & Discussions" icon={<FileText size={15} />}>
            <div className="space-y-2.5">
              {editedData.agendaItems?.map((item: any, i: number) => (
                <div key={i} className="p-3 rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-slate-50/50 dark:bg-white/[0.02] space-y-1.5">
                  {isEditing ? (
                    <div className="space-y-2">
                      ...
                      />
                      <div>
                        <label className="text-[10px] font-600 text-slate-600 dark:text-white/40 mb-0.5 block">Discussion</label>
                        <textarea
                          className="w-full p-2 h-16 bg-white dark:bg-black/20 border border-slate-300 dark:border-white/10 rounded text-xs text-slate-900 dark:text-white resize-none outline-none focus:border-purple-500"
                      ...
                      <div>
                        <label className="text-[10px] font-600 text-slate-600 dark:text-white/40 mb-0.5 block">Decision</label>
                        <input
                    ...
                    <>
                      <h4 className="font-700 text-purple-700 dark:text-purple-300 text-xs sm:text-sm">{i + 1}. {item.title}</h4>
                      <div className="pl-3 border-l-2 border-purple-400/40 space-y-1.5 pt-0.5">
                        ...
                        {item.decision && (
                          <div className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-xs font-600 text-emerald-800 dark:text-emerald-300">
          ...
          {(editedData.keyDecisions?.length > 0 || isEditing) && (
            <Section title="Key Decisions" icon={<Lightbulb size={15} />}>
              <div className="space-y-2">
                {editedData.keyDecisions?.map((decision: string, i: number) => (
                  <div key={i} className="flex items-start gap-2.5 p-2.5 rounded-xl bg-amber-50/60 dark:bg-amber-500/10 border border-amber-200/80 dark:border-amber-500/20">
                    <span className="w-4.5 h-4.5 rounded-full bg-amber-500 text-white text-[10px] font-700 flex items-center justify-center shrink-0 mt-0.5 shadow-2xs">
                      {i + 1}
                    </span>
                    {isEditing ? (
                      <input
                        className="flex-1 p-1 bg-white dark:bg-black/20 border border-slate-300 dark:border-white/10 rounded text-xs text-slate-900 dark:text-white outline-none focus:border-purple-500"
                        value={decision}
                        onChange={(e) => {
          ...
          {/* Action Items (Compact 1-Column Table showing Task only) */}
          <Section title="Action Items" icon={<Target size={15} />}>
            <div className="rounded-xl border border-slate-200/80 dark:border-white/[0.08] overflow-hidden shadow-2xs">
              <table className="w-full text-xs text-left">
                <thead className="bg-slate-100/90 dark:bg-white/[0.04] border-b border-slate-200/80 dark:border-white/[0.08]">
                  <tr>
                    <th className="px-4 py-2 font-700 text-slate-700 dark:text-white/80">Task</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200/60 dark:divide-white/[0.06] bg-white dark:bg-transparent">
                  {editedData.actionItems?.map((a: any, i: number) => (
                    <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-2 font-600 text-slate-900 dark:text-white">
                        {isEditing ? (
                          <input
                            ...
                            value={typeof a === "string" ? a : a.task}
                            onChange={(e) => {
                              const arr = [...editedData.actionItems];
                              arr[i] = typeof arr[i] === "object" ? { ...arr[i], task: e.target.value } : e.target.value;
                  ...
                  {(!editedData.actionItems || editedData.actionItems.length === 0) && (
                    <tr><td className="px-4 py-3.5 text-center text-slate-500 dark:text-white/40">No action items assigned.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
            <Section title="Next Meeting" icon={<FileText size={15} />}>
              <div className="p-3 rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-slate-50/50 dark:bg-white/[0.02]">
                {isEditing ? (
                  <input
                    className="w-full bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-xl px-3 py-1.5 text-xs text-slate-900 dark:text-white outline-none focus:border-purple-500"
            ...
            <Section title="Closing Remarks" icon={<FileText size={15} />}>
              <div className="p-3 rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-slate-50/50 dark:bg-white/[0.02]">
                {isEditing ? (
                  <textarea
                    className="w-full h-16 bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-xl px-3 py-1.5 text-xs text-slate-900 dark:text-white resize-none outline-none focus:border-purple-500"
  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-center gap-2">
        <div className="w-6.5 h-6.5 rounded-lg flex items-center justify-center bg-purple-100 text-purple-700 border border-purple-200 dark:bg-purple-500/20 dark:text-purple-300 dark:border-purple-500/30 shrink-0">
          {icon}
        </div>
        <h3 className="text-sm sm:text-base font-700 text-slate-900 dark:text-white tracking-tight">
          {title}
        </h3>
      </div>
      {children}
...
function MetaCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="p-2.5 rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-slate-50/80 dark:bg-white/[0.02]">
      <div className="flex items-center gap-1.5 mb-0.5">
        {icon}
        <p className="text-[10px] font-600 text-slate-500 dark:text-white/40 uppercase tracking-wider">{label}</p>
      </div>
